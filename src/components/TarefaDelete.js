...
const TarefaDelete = ({ tarefaData, reload, setReload }) => {
  const [isLoading, setIsLoading] = useState(false);

  async function deleteTarefa() {
    try {
      setIsLoading(true);
      await api.delete(`/tarefa/delete/${tarefaData._id}`);
      const tempo = (ms) => {
        ...
        <Modal.Body>
          <Container>
            {isLoading ? (
              ...
              <Row>
                Deseja realmente excluir a tarefa? Essa operação é irreversível.
              </Row>
            )}
          </Container>
        </Modal.Body>
          ...
          <Container>
            <Row>
              <Col className="col-md-6 text-center">
                <Button variant="danger" type="submit" onClick={deleteTarefa}>
