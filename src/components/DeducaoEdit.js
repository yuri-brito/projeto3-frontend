...
const DeducaoEdit = ({ deducaoData, reload, setReload }) => {
  ...
  const handleShow = () => {
    setForm({
      titulo: deducaoData.titulo,
      descricao: deducaoData.descricao,
    });
    setShow(true);
  };
  const handleClose = () => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      setIsLoading(true);
      await api.put(`/deducao/edit/${deducaoData._id}`, form);
      const tempo = (ms) => {
        return new Promise((resolve) => setTimeout(resolve, ms));
      };
      await tempo(2000);
      setIsLoading(false);
      toast.success(`Dados de "${deducaoData.titulo}" alterados com sucesso.`, {
        duration: 7000,
      });
      handleClose();
      setReload(!reload);
    ...
    <div>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>{`Editar: ${deducaoData.titulo}`}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Container>
            {isLoading ? (
